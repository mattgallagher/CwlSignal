...
class SignalTests: XCTestCase {
	func testBasics() {
		...
		let (i1, ep) = Signal<Int>.createInput { $0.subscribe { r in results.append(r) } }
		...
		let (i2, ep2) = Signal<Int>.createInput { $0.transform { r, n in n.send(result: r) }.subscribe { r in results.append(r) } }
	...
	func testSignalPassthrough() {
		...
		let (input, s) = Signal<Int>.createInput()
	...
	func testCapture() {
		...
		var results = [Result<Int>]()
		let (subsequentInput, subsequentSignal) = Signal<Int>.createInput()
	...
	func testCaptureOnError() {
		...
		var results2 = [Result<Int>]()
		let (subsequentInput2, subsequentSignal2) = Signal<Int>.createInput()
	...
	func testDisconnector() {
		...
		do {
			let (i1, s) = Signal<Int>.createInput()
		...
		var results2 = [Result<Int>]()
		let (i4, ep2) = Signal<Int>.createInput { $0.subscribe {
			results2.append($0)
		} }
	...
	func testGraphLoop() {
		...
		do {
			let (input1, signal1) = Signal<Int>.createInput()
			var (input2, signal2) = Signal<Int>.createInput()
			weakInput1 = input1
			weakInput2 = input2
			weakSignal1 = signal1
			weakSignal2 = signal2
			...
			let combined = signal1.combine(second: signal2) { (cr: CombinedResult2<Int, Int>, next: SignalNext<Int>) in
				switch cr {
				case .result1(let r): next.send(result: r)
				case .result2(let r): looped.append(r)
				}
			}.transform { r, n in n.send(result: r) }.continuousOnNext()
			do {
	...
	func testClosedTriangleGraphLeft() {
		var results = [Result<Int>]()
		let (input, signal) = Signal<Int>.createInput { s in s.multicast() }
	...
	func testClosedTriangleGraphRight() {
		var results = [Result<Int>]()
		let (input, signal) = Signal<Int>.createInput { s in s.multicast() }
		let (mergeSet, ep) = Signal<Int>.mergeSetAndSignal([signal], closesOutput: true) { s in s.subscribe { r in results.append(r) } }
	...
	func testMergeSet() {
		do {
			...
			let (input, ep) = Signal<Int>.createInput { $0.subscribe { r in results.append(r) } }
			let disconnector = try mergeSignal.join(toInput: input)
			
			let (input1, signal1) = Signal<Int>.createInput { $0.cacheUntilActive() }
			let (input2, signal2) = Signal<Int>.createInput { $0.cacheUntilActive() }
			let (input3, signal3) = Signal<Int>.createInput { $0.cacheUntilActive() }
			...
			XCTAssert(results.at(4)?.value == 8)
	...
	func testCombine2() {
		...
		let (input1, signal1) = Signal<Int>.createInput()
		let (input2, signal2) = Signal<Double>.createInput()
		
		let (context, specificKey) = Exec.syncQueueWithSpecificKey()
		let combined = signal1.combine(second: signal2, context: context) { (cr: CombinedResult2<Int, Double>, n: SignalNext<String>) in
			XCTAssert(DispatchQueue.getSpecific(key: specificKey) != nil)
			switch cr {
	...
	func testCombine2WithState() {
		...
		let (context, specificKey) = Exec.syncQueueWithSpecificKey()
		let combined = signal1.combine(withState: "", second: signal2, context: context) { (state: inout String, cr: CombinedResult2<Int, Double>, n: SignalNext<String>) in
			XCTAssert(DispatchQueue.getSpecific(key: specificKey) != nil)
			state += "\(results.count)"
			switch cr {
	...
	func testCombine3() {
		...
		let (input1, signal1) = Signal<Int>.createInput()
		let (input2, signal2) = Signal<Double>.createInput()
		let (input3, signal3) = Signal<Int8>.createInput()
		
		let (context, specificKey) = Exec.syncQueueWithSpecificKey()
		let combined = signal1.combine(second: signal2, third: signal3, context: context) { (cr: CombinedResult3<Int, Double, Int8>, n: SignalNext<String>) in
			XCTAssert(DispatchQueue.getSpecific(key: specificKey) != nil)
			switch cr {
			case .result1(.success(let v)): n.send(value: "1 v: \(v)")
			case .result1(.failure(let e)): n.send(value: "1 e: \(e)")
	...
	func testCombine3WithState() {
		...
		let (context, specificKey) = Exec.syncQueueWithSpecificKey()
		let combined = signal1.combine(withState: "", second: signal2, third: signal3, context: context) { (state: inout String, cr: CombinedResult3<Int, Double, Int8>, n: SignalNext<String>) in
			XCTAssert(DispatchQueue.getSpecific(key: specificKey) != nil)
			state += "\(results.count)"
			switch cr {
			case .result1(.success(let v)): n.send(value: "1 v: \(v) \(state)")
	...
	func testCombine4() {
		...
		let (input2, signal2) = Signal<Double>.createInput()
		let (input3, signal3) = Signal<Int8>.createInput()
		let (input4, signal4) = Signal<Int16>.createInput()
		
		let (context, specificKey) = Exec.syncQueueWithSpecificKey()
		let combined = signal1.combine(second: signal2, third: signal3, fourth: signal4, context: context) { (cr: CombinedResult4<Int, Double, Int8, Int16>, n: SignalNext<String>) in
			XCTAssert(DispatchQueue.getSpecific(key: specificKey) != nil)
			switch cr {
			case .result1(.success(let v)): n.send(value: "1 v: \(v)")
			case .result1(.failure(let e)): n.send(value: "1 e: \(e)")
	...
	func testCombine4WithState() {
		...
		let (context, specificKey) = Exec.syncQueueWithSpecificKey()
		let combined = signal1.combine(withState: "", second: signal2, third: signal3, fourth: signal4, context: context) { (state: inout String, cr: CombinedResult4<Int, Double, Int8, Int16>, n: SignalNext<String>) in
			XCTAssert(DispatchQueue.getSpecific(key: specificKey) != nil)
			state += "\(results.count)"
			switch cr {
			case .result1(.success(let v)): n.send(value: "1 v: \(v) \(state)")
	...
	func testCombine5() {
		...
		let (input3, signal3) = Signal<Int8>.createInput()
		let (input4, signal4) = Signal<Int16>.createInput()
		let (input5, signal5) = Signal<Int32>.createInput()
		
		let (context, specificKey) = Exec.syncQueueWithSpecificKey()
		let combined = signal1.combine(second: signal2, third: signal3, fourth: signal4, fifth: signal5, context: context) { (cr: CombinedResult5<Int, Double, Int8, Int16, Int32>, n: SignalNext<String>) in
			XCTAssert(DispatchQueue.getSpecific(key: specificKey) != nil)
			switch cr {
			case .result1(.success(let v)): n.send(value: "1 v: \(v)")
			case .result1(.failure(let e)): n.send(value: "1 e: \(e)")
	...
	func testCombine5WithState() {
		...
		let (context, specificKey) = Exec.syncQueueWithSpecificKey()
		let combined = signal1.combine(withState: "", second: signal2, third: signal3, fourth: signal4, fifth: signal5, context: context) { (state: inout String, cr: CombinedResult5<Int, Double, Int8, Int16, Int32>, n: SignalNext<String>) in
			XCTAssert(DispatchQueue.getSpecific(key: specificKey) != nil)
			state += "\(results.count)"
			switch cr {
			case .result1(.success(let v)): n.send(value: "1 v: \(v) \(state)")
	...
	func testDeepSyncPerformance() {
			...
			let expected = 0.2 // +/- 0.15
			let upperThreshold = 1.0
		#else
			let sequenceLength = 100_000
			let expected = 4.4 // +/- 0.4
