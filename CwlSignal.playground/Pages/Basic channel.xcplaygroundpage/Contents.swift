...
The SignalInput/Signal types returned from createPair are the two ends of the channel and can be passed around your program to locations where values are emitted or where values are needed.
...
// You'd normally store the endpoint in a surrounding scope but we don't have one here.
...
[Next page: Serial pipelines](@next)
